...
export function createAdapter({ profile, log }) {
  ...
  const fetchProps = async (current, model, signal, autoload = false) => {
    ...
    const deadline = combine(signal, current.probeTimeoutMs ?? PROBE_TIMEOUT_MS)
    let response
    try {
  ...
  const resolve = async (current, model, allowProbe, signal) => {
    ...
    return {
      ...
      defaultMaxTokens: current.maxTokens
        ?? maxTokens
        ?? (contextWindow === undefined ? FALLBACK_MAX_TOKENS : outputCapFor(contextWindow)),
